...
class NextConfig_Gallery extends Next_Widget{
    ...
    static function get_next_name() {
        return 'nextaddons-gallery';
    }

    static function get_next_title() {
        return esc_html__( 'Gallery', 'next-addons' );
    }

    static function get_next_icon() {
        return 'nexticons nx-addons nx-addons-gallery-11'; // addons-pro
    }
	static function get_next_keywords() {
       return [ 'Gallery', 'Video', 'Photos Gallery', 'Youtube', 'Vimeo', 'DailyMotion'];
    }
    ...
    static function get_next_dir( $type = '') {
        if($type == 'pro'){
            if ( did_action( 'nextaddonsPro/loaded' ) ) {
                return \NextAddonsPro\Widgets\Manifest::_widgets_dir() . 'gallery/';
            }
        }
        return Manifest::_widgets_dir() . 'gallery/';
    }
    
    static function get_next_url( $type = '') {
        if($type == 'pro'){
            if ( did_action( 'nextaddonsPro/loaded' ) ) {
                return \NextAddonsPro\Widgets\Manifest::_widgets_url() . 'gallery/';
            }
        }
        return Manifest::_widgets_url() . 'gallery/';
    }
    ...
    public function next_scripts(){
        wp_register_style( 'nextaddons-gallery', self::get_next_url() . 'assets/css/gallery.css', false, self::__version() );
       
    }

    public function inline_css( $css = ''){
        wp_add_inline_style( 'nextaddons-gallery', $css);
    }

    public function _get_video_data_new( $type, $url ) {
        
        $return = apply_filters('nextaddons_videos_geturl', $type, $url);
    ...
    public function _embaed_url( $type, $url_video, $settings){
        ...
        $url = apply_filters('nextaddons_videos_getembaded_url', $type, $url_video, $settings);
