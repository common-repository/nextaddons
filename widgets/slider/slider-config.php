...
class NextConfig_Slider extends Next_Widget{
    ...
    static function get_next_name() {
        return 'nextaddons-slider';
    }

    static function get_next_title() {
        return esc_html__( 'Slider Gallery', 'next-addons' );
    }

    static function get_next_icon() {
        return 'nexticons nx-addons nx-addons-gallery-31'; 
    }
	static function get_next_keywords() {
       return [ 'Gallery', 'Slider', 'Video', 'Slider Gallery', 'Youtube', 'Vimeo', 'DailyMotion'];
    }
    ...
    static function get_next_dir( $type = '') {
        if($type == 'pro'){
            if ( did_action( 'nextaddonsPro/loaded' ) ) {
                return \NextAddonsPro\Widgets\Manifest::_widgets_dir() . 'slider/';
            }
        }
        return Manifest::_widgets_dir() . 'slider/';
    }
    
    static function get_next_url( $type = '') {
        if($type == 'pro'){
            if ( did_action( 'nextaddonsPro/loaded' ) ) {
                return \NextAddonsPro\Widgets\Manifest::_widgets_url() . 'slider/';
            }
        }
        return Manifest::_widgets_url() . 'slider/';
    }
    ...
    public function next_scripts(){
        wp_register_style( 'nextaddons-slider', self::get_next_url() . 'assets/css/slider-gallery.css', false, self::__version() );
        wp_register_style( 'nextaddons-slider-pro', self::get_next_url() . 'assets/css/slider-pro.css', false, self::__version() );
    
    }

    public function inline_css( $css = ''){
        wp_add_inline_style( 'nextaddons-slider', $css);
    }

    public function _get_video_data_new( $type, $url ) {
        
        $return = apply_filters('nextaddons_slider_geturl', $type, $url);
    ...
    public function _embaed_url( $type, $url_video, $settings){
        ...
        $url = apply_filters('nextaddons_slider_getembaded_url', $type, $url_video, $settings);
