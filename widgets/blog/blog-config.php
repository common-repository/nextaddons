...
class NextConfig_Blog extends Next_Widget{
    ...
    static function get_next_name() {
        return 'nextaddons-blog';
    }

    static function get_next_title() {
        return esc_html__( 'Blog', 'next-addons' );
    }

    static function get_next_icon() {
        return 'nexticons nx-addons nx-addons-writing'; // addons-pro
    }
    ...
    static function get_next_dir( $type = '') {
        if($type == 'pro'){
            if ( did_action( 'nextaddonsPro/loaded' ) ) {
                return \NextAddonsPro\Widgets\Manifest::_widgets_dir() . 'blog/';
            }
        }
        return Manifest::_widgets_dir() . 'blog/';
    }
    
    static function get_next_url( $type = '') {
        if($type == 'pro'){
            if ( did_action( 'nextaddonsPro/loaded' ) ) {
                return \NextAddonsPro\Widgets\Manifest::_widgets_url() . 'blog/';
            }
        }
        return Manifest::_widgets_url() . 'blog/';
    }
    ...
    public function next_scripts(){
        wp_register_style( 'nextaddons-blog', self::get_next_url() . 'assets/css/blog.css', false, self::__version() );
    }

    public function inline_css( $css = ''){
        wp_add_inline_style( 'nextaddons-blog', $css);
    }
