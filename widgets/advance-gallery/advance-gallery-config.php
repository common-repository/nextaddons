...
class NextConfig_Advance_Gallery extends Next_Widget{
    ...
    static function get_next_name() {
        return 'nextaddons-advance-gallery';
    }

    static function get_next_title() {
        return esc_html__( 'Tab Gallery', 'next-addons' );
    }

    static function get_next_icon() {
        return 'nexticons nx-addons nx-addons-gallery-4'; // addons-pro
    }
	static function get_next_keywords() {
       return [ 'Gallery', 'Video', 'Photos Gallery', 'Youtube', 'Vimeo', 'DailyMotion', 'mixin', 'advance gallery'];
    }
    ...
    static function get_next_dir( $type = '') {
        if($type == 'pro'){
            if ( did_action( 'nextaddonsPro/loaded' ) ) {
                return \NextAddonsPro\Widgets\Manifest::_widgets_dir() . 'advance-gallery/';
            }
        }
        return Manifest::_widgets_dir() . 'advance-gallery/';
    }
    
    static function get_next_url( $type = '') {
        if($type == 'pro'){
            if ( did_action( 'nextaddonsPro/loaded' ) ) {
                return \NextAddonsPro\Widgets\Manifest::_widgets_url() . 'advance-gallery/';
            }
        }
        return Manifest::_widgets_url() . 'advance-gallery/';
    }
    ...
    public function next_scripts(){
        wp_register_style( 'nextaddons-mixingallery', self::get_next_url() . 'assets/css/mixin-gallery.css', false, self::__version() );
       
    }

    public function inline_css( $css = ''){
        wp_add_inline_style( 'nextaddons-mixingallery', $css);
    }

    public function _get_video_data_new( $type, $url ) {
        
        $return = apply_filters('nextaddons_advideos_geturl', $type, $url);
    ...
    public function _embaed_url( $type, $url_video, $settings){
        ...
        $url = apply_filters('nextaddons_advideos_getembaded_url', $type, $url_video, $settings);
